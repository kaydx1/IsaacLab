...
from isaaclab_assets.robots.booster import BOOSTER_T1_CFG  # isort: skip
...
class CommandsCfg:
    ...
    base_velocity = mdp.GaitCommandCfg(
        ...
        ranges=mdp.GaitCommandCfg.Ranges(
            ...
            lin_vel_y=(-1.0, 1.0), 
            ang_vel_yaw=(-1.0, 1.0),
            gait_frequency=(1.0, 2.0) # Example range for gait frequency in Hz
        ),
    )
...
class ActionsCfg:
    ...
    joint_pos = mdp.JointPositionActionCfg(
        asset_name="robot",
        joint_names=[".*"],  # applies to all joints
        scale=0.25,
        use_default_offset=True,
        # clip={".*": (-1.0, 1.0)}  # clip between -1 and 1
    )
...
class T1Rewards:
    ...
    tracking_lin_vel_xy = RewTerm(
        ...
        weight=3.0,  # Corresponds to tracking_lin_vel_x and tracking_lin_vel_y
    ...
    root_acc = RewTerm(
        ...
        weight=-1.0e-4,
        params={"asset_cfg": SceneEntityCfg("robot", body_names="Waist")} # Use the name of your root body
    )


    # -- Effort and Joint Penalties --
    torques = RewTerm(func=mdp.joint_torques_l2, weight=-0.5e-4)
    ...
    feet_slip = RewTerm(
        ...
        weight=-0.1,
        params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_foot_link")} # Modify regex for your foot names
    )
    ...
    joint_deviation_torso = RewTerm(
        func=mdp.joint_deviation_l1, weight=-0.3, params={"asset_cfg": SceneEntityCfg("robot", joint_names="Waist")}
    )

    joint_deviation_hip_yaw= RewTerm(
        func=mdp.joint_deviation_l1, weight=-0.3, params={"asset_cfg": SceneEntityCfg("robot", joint_names=".*_Hip_Yaw")}
    )

    joint_deviation_head= RewTerm(
        func=mdp.joint_deviation_l1, weight=-0.3, params={"asset_cfg": SceneEntityCfg("robot", joint_names=["AAHead_yaw", "Head_pitch"])}
    )
